...
const initialState = { tools: [], categories: [], isCreateToolDialog: false, isCreateCategoryDialog: false, createErrorMessage: '', curTabId: false } as ToolsPageState;
...
const tools = createSlice({
  ...
  reducers: {
    ...
    updateCategories: (state, action) => {
      ...
      if (state.categories.length != 0) state.curTabId = state.categories[0].categoryId;
    },
...
export const { addCategoryRequest, addToolRequest, getToolsRequest, getCategoriesRequest, removeToolRequest, removeCategoryRequest, updateCreateToolDialog, 
  updateCreateCategoryDialog, updateCreateErrorMessage, updateCurTabId } = tools.actions;
